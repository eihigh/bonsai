...
func (g *Game) Update() error {
	g.inputState = ebiten.CurrentInputState() //
	return nil
}
...
// elapsed during the Until or Loop period, in the range [0, 1).
...
func (t Tick) Wait(duration int) Tick {
	if t.t < duration {

	}
}

// Until calls f only within the span.
...
func (t Tick) Until(duration int, f func(Tick)) Tick {
...
// Loop continues to call function f after the start tick has elapsed,
// where t is the tick to be reset for each duration tick and
// n is the number of iterations.
func (t Tick) Loop(duration int, f func(int, Tick)) {
	e := t.t - start
	if e < 0 {
		return
	}
...
func (t Tick) LoopFor(duration, times int, f func(int, Tick)) Tick {
	return t.Until(duration*times, func(t Tick) {
		t.Loop(0, duration, func(n int, t Tick) {
			f(n, t)
		})
	})
}

func (t Tick) EveryFor(duration, times int, f func(int)) Tick {
	return t.Until(duration*times, func(t Tick) {
