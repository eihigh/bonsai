...
func ExampleTick_Span() {
	...
	for i := 0; i < 9; i++ {
		...
		// Chained spans.
		t.Until(3, func(t Tick) { // tick 0, 1, 2
			e1 := t.Elapsed()
			fmt.Println(e0, e1)
		}).Wait(2).Until(4, func(t Tick) { // tick 5, 6, 7, 8 (3, 4 are skipped)
			e1 := t.Elapsed()
			fmt.Println(e0, e1)
		})

		// Another span.
		t.Wait(2).Until(4, func(t Tick) { // tick 2, 3, 4, 5
...
func ExampleTick_Loop() {
	...
	for i := 0; i < 10; i++ {
		...
		t.Wait(2).Loop(3, func(n int, t Tick) { // tick 2, 3, 4; 5, 6, 7; 8, 9, 10;
			e1 := t.Elapsed()
			fmt.Println(e0, n, e1)
		})
	}
...
func ExampleTick_LoopFor() {
	...
	for i := 0; i < 10; i++ {
		...
		t.Wait(1).LoopFor(3, 2, func(n int, t Tick) { // tick 1, 2, 3; 4, 5, 6
			e1 := t.Elapsed()
			fmt.Println(e0, n, e1)
		}).Once(0, func() {
			fmt.Println(e0, "done")
...
func ExampleTick_EveryFor() {
	...
	for i := 0; i < 10; i++ {
		...
		t.Wait(1).EveryFor(2, 3, func(n int) { // tick 1, 3, 5
			fmt.Println(e0, n)
		}).Once(0, func() {
			fmt.Println(e0, "done")
		})
